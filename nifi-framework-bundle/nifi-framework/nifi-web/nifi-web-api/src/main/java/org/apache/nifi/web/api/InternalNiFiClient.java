...
public class InternalNiFiClient implements NiFiClientOperations {
    ...
    public void deleteConnection(final String connId) {
        try {
            ...
            if (entity.getComponent() != null) {
                stopConnectable(entity.getComponent().getSource());
                stopConnectable(entity.getComponent().getDestination());
            }
        } catch (RuntimeException e) {
    ...
    private void stopConnectable(final ConnectableDTO connectable) {
        if (connectable == null || connectable.getId() == null) return;
        if ("PROCESSOR".equalsIgnoreCase(connectable.getType())) {
            try {
                stopProcessor(connectable.getId());
