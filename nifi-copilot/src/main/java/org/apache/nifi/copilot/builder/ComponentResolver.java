...
final class ComponentResolver {
    ...
    void registerExistingProcessors(
        ...
        for (Map.Entry<String, String> entry : existingIdMap.entrySet()) {
            final String specId = entry.getKey();
            final String nifiId = entry.getValue();
            if (nifiId != null && !nifiId.isBlank()) {
                try {
                    components.register(specId, nifiId, "PROCESSOR");
                } catch (IllegalStateException e) {
                    // Ignore if already registered with the same or different NiFi ID (e.g., from canvas read)
                }
            }
        }
    }
