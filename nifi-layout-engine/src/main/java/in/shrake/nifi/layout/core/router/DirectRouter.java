...
public class DirectRouter implements RoutingStrategy {
    ...
    public RoutingResult computeRoutes(LayoutGraph graph, CoordinateAssignment coordinates, LayoutOptions options) {
        ...
        for (LayoutEdge edge : graph.getEdges().values()) {
            BoundingBox sourceBb = coordinates.getBounds().get(edge.getSourceNodeId());
            BoundingBox targetBb = coordinates.getBounds().get(edge.getTargetNodeId());
            
            if (sourceBb == null || targetBb == null) {
                continue;
            }

            // NiFi calculates terminal anchors from component bounds. A direct route therefore
            // has no explicit bend points.
