...
export class CopilotChatComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        this.http
            .get<{ externalUrl: string | null }>(`${this.backendUrl}/api/config`)
            .pipe(catchError(() => of({ externalUrl: null })))
            .subscribe((config) => {
                if (config.externalUrl) {
                    this.backendUrl = config.externalUrl + this.backendUrl;
                }
